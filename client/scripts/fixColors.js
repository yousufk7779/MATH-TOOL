...
svgFiles.forEach((file) => {
  ...
  const regex =
    /(?:stroke|fill)(?::|\s*=\s*["']?)\s*(#(?:[A-Fa-f0-9]{3}){1,2}|[a-zA-Z]+)(?![a-zA-Z0-9])/gi;
  let match;
  ...
  while ((match = regex.exec(content)) !== null) {
    ...
    if (
      ...
      stripped !== "#fff" &&
      stripped !== "#ffffff" &&
      stripped !== "transparent" &&
      !darkColors.includes(stripped) &&
  ...
  const replaceRegex = new RegExp(
    `(fill|stroke|color)(?:\\s*:\\s*|\\s*=\\s*(["']))(${darkColors.join("|")})(?![A-Za-z0-9])`,
    "gi",
  );
...
console.log(
  `Processed ${svgFiles.length} SVGs and updated ${modifiedCount} files.`,
);
