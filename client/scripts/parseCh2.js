...
function extractSolutionSteps(solutionHtml) {
  ...
  $("*").each((i, el) => {
    if ($(el).is("img")) {
      ...
      steps.push(
        `<img src="${src}" style="background: #FFFFFF; padding: 10px; border-radius: 10px; width: 100%; max-width: 320px; height: 180px; object-fit: contain; margin: 15px auto; display: block; border: 1px solid #e9ecef; box-shadow: 0 4px 6px rgba(0,0,0,0.05);" />`,
      );
    } else if ($(el).text().trim() && !$(el).children("img").length) {
      // Check if it's purely text content
      if (
        $(el).children().length === 0 ||
        ($(el).children().length === 1 && $(el).children("span").length)
      ) {
  ...
  $(".question-container").each((i, el) => {
    ...
    $(el)
      ...
      .each((j, node) => {
        if (node.tagName === "img") {
          solutionSteps.push(
            `<img src="${$(node).attr("src")}" style="background: #FFFFFF; padding: 10px; border-radius: 10px; width: 100%; max-width: 320px; height: 180px; object-fit: contain; margin: 15px auto; display: block; border: 1px solid #e9ecef; box-shadow: 0 4px 6px rgba(0,0,0,0.05);" />`,
          );
        } else {
