...
async function fixAllTsFiles() {
  ...
  for (const file of files) {
    ...
    const regex =
      /data:image\/svg\+xml(?:;charset=[A-Za-z0-9-]+)?;base64,([A-Za-z0-9+/=]+)/g;
    let match;
    let replacements = [];
    ...
    while ((match = regex.exec(content)) !== null) {
      ...
      try {
        ...
        let pngBuffer = await sharp(svgBuffer, { density: 300 })
          .png()
          .toBuffer();
    ...
    if (replacements.length > 0) {
      ...
      console.log(
        `Replaced ${replacements.length} inline SVGs with PNGs in ${file}.`,
      );
  ...
  console.log(
    `Finished converting ${totalReplaced} SVGs to PNGs across all target TS files.`,
  );
}
