...
html = html.replace(
  /<span class="option">\(([a-d])\)/gi,
  (match, p1) => `<span class="option">(${p1.toUpperCase()})`,
);
html = html.replace(
  /Correct Option: \(([a-d])\)/gi,
  (match, p1) => `Correct Option: (${p1.toUpperCase()})`,
);
...
boxes.forEach((boxContent) => {
  ...
  const optMatch = [
    ...boxContent.matchAll(/<span class="option">([\s\S]*?)<\/span>/g),
  ];
  const ansMatch = boxContent.match(
    /<div class="final-answer">([\s\S]*?)<\/div>/,
  );
  ...
  if (qMatch && optMatch.length >= 4 && ansMatch) {
    ...
    const correctLetter = correctLetterMatch
      ? correctLetterMatch[1].toUpperCase()
      : "";
