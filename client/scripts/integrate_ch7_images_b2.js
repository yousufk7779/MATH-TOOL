...
const brainPath =
  "C:\\Users\\hp\\.gemini\antigravity\\brain\\5f289ee6-cb2d-4d3c-b14f-c5ab390b3671";
...
const imgStyle =
  "background: #FFFFFF; padding: 10px; border-radius: 10px; width: 100%; max-width: 320px; height: 180px; object-fit: contain; margin: 15px auto; display: block; border: 1px solid #e9ecef; box-shadow: 0 4px 6px rgba(0,0,0,0.05);";
...
for (const [id, fileName] of Object.entries(imageMap)) {
  ...
  if (fs.existsSync(fullPath)) {
    ...
    const regex = new RegExp(
      `(id:\\s*"${id}"[\\s\\S]*?)(image:\\s*"[^"]*",)?([\\s\\S]*?solution:\\s*\\[)`,
      "g",
    );
