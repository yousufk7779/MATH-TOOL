...
let result = {
  ...
  definitions: [
    ...
    },
    {
      term: "Discriminant (D)",
      description:
        "Evaluated as b² - 4ac, which determines the nature of the roots.",
    },
  ],
  keyPoints: [
    ...
    $(el)
      ...
      .each((j, child) => {
        ...
        if ($(child).hasClass("sub-question")) {
          ...
          solutionSteps.push(
            `<span style="color: #FFAB00; font-weight: normal;">${cleanText($(child).text())}</span>`,
          );
          ...
          solutionSteps.push(
            `<img src="${$(child).attr("src")}" style="background: #FFFFFF; padding: 10px; border-radius: 10px; width: 100%; max-width: 320px; height: 84px; object-fit: contain; margin: 15px auto; display: block; border: 1px solid #e9ecef; box-shadow: 0 4px 6px rgba(0,0,0,0.05);" />`,
          );
        } else if (
          $(child).hasClass("table-container") ||
          $(child).is("table")
        ) {
          solutionSteps.push(
            '<div class="table-container">' + $(child).html() + "</div>",
          );
        } else {
              ...
              $(child)
                ...
                .each((k, node) => {
                  if (node.tagName === "img") {
                    ...
                    if (text && text !== "»" && text !== "")
                      solutionSteps.push(text.replace(/^»\s*/, ""));
                  } else {
                    let text = cleanText($(node).text());
                    if (text && text !== "»" && text !== "")
                      solutionSteps.push(
...
if (fs.existsSync(mcqPath)) {
  ...
  $(".content-box").each((i, el) => {
    ...
    correctAnsText = correctAnsText.replace(
      /\\\(\\frac\{([^\}]+)\}\{([^\}]+)\}\\\)/g,
      "<sup>$1</sup>&frasl;<sub>$2</sub>",
    );
