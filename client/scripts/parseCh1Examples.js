  ...
  $(el)
    ...
    .each((j, child) => {
      ...
      if ($(child).hasClass("sub-question")) {
        ...
        solutionSteps.push(
          `<img src="${src}" style="background: #FFFFFF; padding: 10px; border-radius: 10px; width: 100%; max-width: 320px; height: 120px; object-fit: contain; margin: 15px auto; display: block; border: 1px solid #e9ecef; box-shadow: 0 4px 6px rgba(0,0,0,0.05);" />`,
        );
      } else {
...
fs.writeFileSync(
  path.join(__dirname, "../temp_examples_ch1.json"),
