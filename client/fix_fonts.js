...
files.forEach(file => {
  ...
  content = content.replace(/\.q-text\s*\{[^}]*\}/g, match => match.replace(/font-size:\s*\d+px;?\s*/g, ''));
  content = content.replace(/\.q-subtext\s*\{[^}]*\}/g, match => match.replace(/font-size:\s*\d+px;?\s*/g, ''));
  content = content.replace(/\.sol-step\s*\{[^}]*\}/g, match => match.replace(/font-size:\s*\d+px;?\s*/g, ''));
  content = content.replace(/\.sol-box\s*\{[^}]*\}/g, match => match.replace(/font-size:\s*\d+px;?\s*/g, ''));

  // To avoid adding the rule multiple times if script is run twice, we clean it up first
  content = content.replace(/\.q-text,\s*\.q-subtext,\s*\.sol-step,\s*\.sol-box\s*\{\s*font-size:\s*16px;\s*\}\s*/g, '');

  // Inject the universal 16px rule for Q&A right before </style>
  const newRule = "    .q-text, .q-subtext, .sol-step, .sol-box { font-size: 16px; }\n  </style>";
