...
const generateQuestions = () => {
    ...
    const categories = [
        ...
        },
        {
            ...
            items: [
                ...
                ["In Heron's formula, a, b, c are:", ["Angles", "Vertices", "Sides", "None"], "C"],
        ...
        },
        {
            ...
            items: [
                ...
                ["The difference between max and min values is:", ["Mean", "Median", "Mode", "Range"], "D"],
                ...
                ["Cumulative frequency table is used to find:", ["Mean", "Median", "Mode", "Range"], "B"]
            ]
        },
        {
            name: "Probability",
    ...
    categories.forEach(cat => {
        cat.items.forEach(item => {
            allRaw.push({q: item[0], opts: item[1], ans: item[2], chapter: cat.name});
        });
    });
    ...
    const finalQuestions = allRaw.map((item, i) => ({
        id: `mq9-${i + 1}`,
        question: item.q,
        options: item.opts.map((opt, j) => `(${String.fromCharCode(65 + j)}) ${opt}`),
        correctAnswer: item.ans,
        chapterId: "math9-mix",
        chapterTitle: item.chapter
    }));

    fs.writeFileSync('d:/All NCERT SOLUTIONS/client/data/easy_mcqs_math_c9.json', JSON.stringify(finalQuestions, null, 2));
