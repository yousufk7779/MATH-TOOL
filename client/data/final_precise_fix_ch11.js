...
mapping.forEach((item) => {
  ...
  if (match) {
    ...
    if (fs.existsSync(fullPath)) {
      ...
      // CLEANING
      // Fix double stroke/fill attributes
      svg = svg.replace(/stroke="stroke="([^"]+)"/g, 'stroke="$1"');
      svg = svg.replace(/fill="none""/g, 'fill="none"');

      // Remove redundant attributes often found together
      // e.g. stroke="black" stroke-width="2" fill="none" stroke-width="2" fill="none"
      svg = svg.replace(
        /(stroke="[^"]+" stroke-width="[^"]+" fill="[^"]+")\s+\1/g,
        "$1",
      );
      ...
      console.log(`Cleaned and re-embedded ${item.file}`);
    }
  } else {
    console.warn(`Block not found: ${item.key}`);
  }
});

fs.writeFileSync(tsFile, content, "utf8");
console.log("Chapter 11 finalized.");
