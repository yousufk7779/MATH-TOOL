...
function Class8SocialScienceScreen() {
  const navigation = useNavigation<NavigationProp>();
  const route =
    useRoute<RouteProp<RootStackParamList, "Class8SocialScience">>();
  const { width, height } = useWindowDimensions();
  const isLandscape = width > height;

  const className = "Class 8";
  const subjectName = "Class 8 Social Science";
...
export default memo(Class8SocialScienceScreen);
