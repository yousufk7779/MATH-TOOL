...
function Class9SocialScienceScreen() {
  const navigation = useNavigation<NavigationProp>();
  const route =
    useRoute<RouteProp<RootStackParamList, "Class9SocialScience">>();
  const { width, height } = useWindowDimensions();
  const isLandscape = width > height;

  const className = "Class 9";
  const subjectName = "Class 9 Social Science";
...
export default memo(Class9SocialScienceScreen);
