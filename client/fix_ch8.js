...
if (q5Match) {
  ...
  if (solution.includes("Example 6.")) {
    ...
    let endMatch = solution.match(
      /("\*\*Therefore, the required values are [\s\S]*?\.\*\*")/,
    );
    if (endMatch) {
      let cleanSolution = solution.substring(
        0,
        solution.indexOf(endMatch[1]) + endMatch[1].length,
      );
...
// Concepts
content = content.replace(
  /\\(Perp|Base|Hyp|Adj|Perpendicular|Base|Hypotenuse)/g,
  "$1",
);
