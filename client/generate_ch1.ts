...
import { mathCh1 } from "./data/content/math-ch1";

const HTML_DIR = path.join(__dirname, "assets", "chapters", "chapter1");
const OUTPUT_FILE = path.join(__dirname, "data", "content", "math-ch1.ts");
...
export const mathCh1: ChapterContent = {
  ...
  number: 1,
  title: "Real Numbers",
  introduction: ${JSON.stringify(mathCh1.introduction)},
  definitions: ${JSON.stringify(mathCh1.definitions, null, 2)},
  keyPoints: ${JSON.stringify(mathCh1.keyPoints, null, 2)},
  formulas: ${JSON.stringify(mathCh1.formulas, null, 2)},
  crux: ${JSON.stringify(mathCh1.crux, null, 2)},
  summary: ${JSON.stringify(mathCh1.summary, null, 2)},
  examples: ${JSON.stringify(mathCh1.examples, null, 2)},
  exercises: [
    { id: "examples", name: "Examples", questions: [] },
    { id: "exercise1", name: "Exercise 1.1", questions: [] },
    { id: "exercise2", name: "Exercise 1.2", questions: [] }
  ],
  mcqs: ${JSON.stringify(mathCh1.mcqs, null, 2)},
  isHtmlView: true,
  htmlExercises: {
    examples: \`${examples}\`,
    exercise1: \`${exercise1}\`,
...
console.log("math-ch1.ts generated successfully with WebView HTML support and standard native MCQs!");
