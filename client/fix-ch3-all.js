...
async function run() {
  ...
  jsonStr = jsonStr
    .substring(0, jsonStr.lastIndexOf(";") || jsonStr.length)
    .trim();
  ...
  async function processHtmlStr(str) {
    ...
    if (svgMatch) {
      for (const match of svgMatch) {
        ...
        try {
          const pngBuffer = await sharp(Buffer.from(b64, "base64"))
            .png()
            .toBuffer();
    ...
    if (newStr.includes("<table")) {
      ...
      $("table").each((_, tbl) => {
        let divStr =
          '<div style="background-color:rgba(255,255,255,0.05); border-radius:8px; border:1px solid rgba(255,255,255,0.2); margin-top:10px; margin-bottom:10px; overflow:hidden;">';
