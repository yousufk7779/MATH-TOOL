...
function cleanLine(l) {
  ...
  l = l.replace(
    /^(Write given expression:|Put values:|Multiply values:|Add them:|Simplify denominator:|Multiply:|Take LCM[^:]*:|Cancel[^:]*:|Simplify numerator:|Simplify squares:|Divide by 2:|Final value:)\s*/i,
    "= ",
  );
  ...
  l = l.replace(/^=\s*=\s*/g, "= ");
  l = l.replace(
    /^Since ([^,]+),\s*(the\s+)?correct option is\s*(.*?)\.?$/i,
    "Since $1, correct option is $3",
  );

  // Fractional replacements
  let fractionRegex =
    /<sup>((?:(?!<sup|<sub).)*?)<\/sup>\/<sub>((?:(?!<sup|<sub).)*?)<\/sub>/g;
