...
export const generateQuizAsync = async (
  ...
  if (className === "Class 7") {
    if (subject === "Mathematics") {
      const mathMcqs = easyMathC7Mcqs as QuizQuestion[];
  ...
  if (className === "Class 8") {
    if (subject === "Mathematics") {
      const mathMcqs = easyMathC8Mcqs as QuizQuestion[];
      return shuffleArray(mathMcqs).slice(0, questionCount);
    }
    if (subject === "Science") {
      ...
      const r1 = shuffleArray(remaining).slice(
        0,
  ...
  if (subject === "Mathematics") {
    return shuffleArray(easyMathsMcqs as QuizQuestion[]).slice(
      0,
      questionCount,
    );
  }
  ...
  if (subject === "Science") {
    ...
    const r4 = shuffleArray(remaining).slice(
      0,
      questionCount - selectedIds.size,
    );
  ...
  if (subject === "Social Science") {
    const sstMcqs = easySstMcqs as QuizQuestion[];
    return shuffleArray(sstMcqs).slice(0, questionCount);
  }
