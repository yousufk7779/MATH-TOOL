...
<script>
  (function() {
    // Inject styles dynamically to guarantee they are applied even if WebView strips body styles
    ...
    function initQuiz() {
      try {
        ...
        boxes.forEach(box => {
          ...
          // Force hide elements robustly inside the loop too
          ...
          options.forEach((opt, idx) => {
            ...
            // Bind explicitly via both touchstart and click to avoid mobile webview delays or swallowing
        ...
        // Fallback for debugging, if there's an error, it will just log instead of completely bricking everything
    ...
    // Also try firing after a short delay to ensure React Native hasn't suppressed DOMContentLoaded
    setTimeout(function() {
       if (document.querySelectorAll('.content-box').length > 0 && !document.querySelectorAll('.content-box')[0].dataset.initialized) {
          document.querySelectorAll('.content-box').forEach(b => b.dataset.initialized = "true");
          // Re-fire if somehow missed
          initQuiz();
       }
    }, 1000);
