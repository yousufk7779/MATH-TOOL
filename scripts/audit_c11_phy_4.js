...
const file = path.join(__dirname, '../client/data/content/c11-phy-4.ts');
const content = fs.readFileSync(file, 'utf-8');

console.log('=== AUDITING CLASS 11 PHYSICS CHAPTER 4 ===\n');
...
const qMatches = content.match(/<h3 style=\\"color: #00c6ff; margin: 0 0 10px 0; font-size: 17px; font-weight: bold; line-height: 1\.5;\\">Q\d+:/g) || [];
...
const vsaHeaders = content.match(/Section A: Very Short Answer \(VSA\) Questions \(1 Mark each\) &bull; Q1 to Q40/);
const saHeaders = content.match(/Section B: Short Answer \(SA\) Questions \(2 & 3 Marks each\) &bull; Q41 to Q80/);
const laHeaders = content.match(/Section C: Long Answer \(LA\) Questions \(5 Marks each\) &bull; Q81 to Q100/);
...
const mcqRegex = /"id": "c11-phy-4-mcq-(\d+)"/g;
...
const hasImport = ccContent.includes('c11Phy4HtmlOverview, c11Phy4HtmlSolutions, c11Phy4Mcqs } from "./content/c11-phy-4"');
const hasWiring = ccContent.includes('ch11Phy4.mcqs = c11Phy4Mcqs;');

if (hasImport && hasWiring) {
  console.log('✅ PASSED: client/data/chapterContent.ts correctly imports and connects c11-phy-4 overview, solutions, and mcqs.');
} else {
  console.error('❌ FAILED: client/data/chapterContent.ts wiring is missing or incomplete!');
  failed = true;
}

// 8. Cheat sheet check
const hasCheatSheet = content.includes('Master Revision Formula Cheat Sheet');
if (hasCheatSheet) {
  console.log('✅ PASSED: Master Revision Formula Cheat Sheet present.');
} else {
  console.error('❌ FAILED: Cheat sheet missing in Overview!');
  failed = true;
}
...
if (failed) {
  ...
  console.log('\n🎉 ALL 8 AUDIT CHECKS PASSED WITH FLYING COLORS!');
}
