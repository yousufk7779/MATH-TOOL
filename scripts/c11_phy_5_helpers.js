// Helper components and theme configuration for Class 11 Physics Chapter 5: Motion of System of Particles and Rigid Body
...
function defBox(term, def, unit = "", dim = "") {
  return `
  <div style="background: rgba(171, 71, 188, 0.06); border: 1.5px solid ${themeColor}; border-radius: 10px; padding: 14px 18px; margin: 16px 0;">
    ...
    ${unit || dim ? `
    <div style="display: flex; gap: 16px; margin-top: 10px; padding-top: 8px; border-top: 1px dashed rgba(171, 71, 188, 0.3); font-size: 13.5px; color: #CBD5E1;">
      ${unit ? `<span><b>SI Unit:</b> <code style="color: #CE93D8; background: rgba(0,0,0,0.3); padding: 1px 6px; border-radius: 4px;">${unit}</code></span>` : ""}
      ${dim ? `<span><b>Dimensional Formula:</b> <code style="color: #CE93D8; background: rgba(0,0,0,0.3); padding: 1px 6px; border-radius: 4px;">${dim}</code></span>` : ""}
    </div>` : ""}
  </div>`;
}
...
function stepDerivationBox(title, steps) {
  ...
  const stepsHtml = steps.map((s, idx) => `
    <div style="display: flex; gap: 12px; margin-bottom: 12px; align-items: flex-start;">
      <span style="background: rgba(171, 71, 188, 0.2); color: ${themeColor}; font-weight: bold; width: 24px; height: 24px; border-radius: 50%; display: flex; align-items: center; justify-content: center; flex-shrink: 0; font-size: 12px; border: 1px solid ${themeColor};">${idx + 1}</span>
      <div style="flex: 1;">
  ...
  return `
  <div style="background: rgba(15, 23, 42, 0.6); border: 1px solid rgba(171, 71, 188, 0.3); border-radius: 10px; padding: 16px; margin: 18px 0;">
    <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 14px; border-bottom: 1px solid rgba(171, 71, 188, 0.2); padding-bottom: 8px;">
...
function solvedExampleBox(num, q, steps, ans) {
  ...
  return `
  <div style="background: rgba(30, 41, 59, 0.7); border: 1px solid rgba(171, 71, 188, 0.4); border-radius: 10px; padding: 16px; margin: 18px 0;">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; border-bottom: 1px solid rgba(171, 71, 188, 0.2); padding-bottom: 8px;">
      <b style="color: ${themeColor}; font-size: 15px;">Solved Board Numerical #${num}</b>
      <span style="background: rgba(171, 71, 188, 0.15); color: ${themeColor}; font-size: 12px; padding: 2px 8px; border-radius: 4px; font-weight: 600;">NCERT / CBSE HOTS</span>
    </div>
