...
console.log('=== AUDITING CLASS 11 PHYSICS CHAPTER 7 (UNIT VII: PROPERTIES OF BULK MATTER) ===\n');

let passed = true;

// 1. Check file existence and size
if (!fs.existsSync(targetFile)) {
  console.error('FAIL: c11-phy-7.ts does not exist!');
  process.exit(1);
}
...
const mcqIdMatches = content.match(/c11-phy-7-mcq-\d+/g);
...
const hasImport = ccContent.includes('c11Phy7HtmlOverview') &&
                  ccContent.includes('c11Phy7HtmlSolutions') &&
                  ccContent.includes('c11Phy7Mcqs');
const hasWiring = ccContent.includes('ch11Phy7.mcqs = c11Phy7Mcqs;');
...
console.log('\n================================================================');
if (passed) {
  console.log('>>> ALL QUALITY GATES PASSED PERFECTLY FOR CHAPTER 7! <<<');
} else {
