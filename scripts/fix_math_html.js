  ...
  for (const file of files) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      content = content.replace(
        /<span\s+class="fraction"\s*>\s*<span\s+class="numerator"\s*>([\s\S]*?)<\/span>\s*<span\s+class="denominator"\s*>([\s\S]*?)<\/span>\s*<\/span>/g,
      ...
      content = content.replace(/\\\([\s\S]*?\\\)/g, (match) => {
        return match.replace(
          /<span class="fraction"><span class="numerator">([\s\S]*?)<\/span><span class="denominator">([\s\S]*?)<\/span><\/span>/g,
          "\\frac{$1}{$2}",
        );
      });
