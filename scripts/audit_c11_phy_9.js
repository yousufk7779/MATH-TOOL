...
console.log('=== AUDITING CLASS 11 PHYSICS CHAPTER 9 (UNIT IX: BEHAVIOUR OF PERFECT GASES AND KINETIC THEORY OF GASES) ===\n');

let passed = true;

// 1. Check file existence and size
if (!fs.existsSync(targetFile)) {
  console.error('FAIL: c11-phy-9.ts does not exist!');
  process.exit(1);
}
...
if (/SECTION A.*Very Short Answer/i.test(content) &&
    /SECTION B.*SHORT ANSWER/i.test(content) &&
    /SECTION C.*Long Answer/i.test(content)) {
...
const mcqIdMatches = content.match(/c11-phy-9-mcq-\d+/g);
...
const hasImport = ccContent.includes('c11Phy9HtmlOverview') &&
                  ccContent.includes('c11Phy9HtmlSolutions') &&
                  ccContent.includes('c11Phy9Mcqs');
const hasWiring = ccContent.includes('ch11Phy9.mcqs = c11Phy9Mcqs;');
...
console.log('\n================================================================');
if (passed) {
  console.log('>>> ALL QUALITY GATES PASSED PERFECTLY FOR CHAPTER 9! <<<');
} else {
