...
for (const file of files) {
  ...
  lines = lines.map((line) => {
    if (
      line.includes('<span class="step">') ||
      line.includes('<span class="answer">')
    ) {
      ...
      if (lastIndex !== -1) {
        modified =
          modified.substring(0, lastIndex) +
          "</div>" +
          modified.substring(lastIndex + 7);
      }
      return modified;
    }
    return line;
  ...
  content = content.replace(
    /<div class="question">\s*\(\s*(i|ii|iii|iv|v|vi|vii|viii|ix|x|xi|xii|xiii|xiv|xv)\s*\)([\s\S]*?)<\/div>/gi,
    '<div class="sub-question">($1)$2</div>',
  );
