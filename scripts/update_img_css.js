...
function processHTMLFiles(dir) {
  ...
  for (const file of files) {
    ...
    if (fs.statSync(fullPath).isDirectory()) {
      ...
      const oldImgCSS =
        /img\s*\{\s*max-width:\s*100%;\s*height:\s*auto;\s*max-height:\s*250px;\s*object-fit:\s*contain;\s*display:\s*block;\s*margin:\s*15px\s*auto;\s*border-radius:\s*8px;\s*\}/g;
      const newImgCSS = `img {
            background: #FFFFFF;
            padding: 10px;
            border-radius: 10px;
