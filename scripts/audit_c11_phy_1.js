...
const file = path.join(__dirname, '../client/data/content/c11-phy-1.ts');
const content = fs.readFileSync(file, 'utf-8');

console.log('=== AUDITING CLASS 11 PHYSICS CHAPTER 1 ===\n');

let failed = false;

// 1. Raw carets check
// Note: exponents should use <sup>, not raw ^ in text
...
const qMatches = content.match(/<h3 style=\\"color: #FF512F; margin: 0 0 10px 0; font-size: 17px; font-weight: bold; line-height: 1\.5;\\">Q\d+:/g) || [];
...
const mcqRegex = /"id": "c11-phy-1-mcq-(\d+)"/g;
...
const ccCode = fs.readFileSync(ccFile, 'utf-8');
const hasImport = ccCode.includes('c11Phy1Mcqs');
const hasAssignment = ccCode.includes('ch11Phy1.mcqs = c11Phy1Mcqs;');

if (hasImport && hasAssignment) {
  console.log('✅ PASSED: chapterContent.ts correctly imports and assigns c11Phy1Mcqs.');
} else {
  console.error('❌ FAILED: chapterContent.ts wiring incomplete! hasImport=' + hasImport + ', hasAssignment=' + hasAssignment);
  failed = true;
}

if (!failed) {
  console.log('\n🎉 ALL QUALITY & BLUEPRINT AUDITS PASSED WITH 100% EXCELLENCE!');
} else {
  console.log('\n⚠️ Some audits failed. Please review errors above.');
}
