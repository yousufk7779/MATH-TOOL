// Helper components and theme configuration for Class 11 Physics Chapter 6: Gravitation
...
function defBox(term, def, unit = "", dim = "") {
  return `
  <div style="background: rgba(253, 200, 48, 0.06); border: 1.5px solid ${themeColor}; border-radius: 10px; padding: 14px 18px; margin: 16px 0;">
    <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 6px;">
      <span style="background: ${themeColor}; color: #1E293B; font-weight: bold; font-size: 11px; padding: 2px 8px; border-radius: 4px; text-transform: uppercase; letter-spacing: 0.5px;">DEFINITION</span>
      <b style="color: ${themeColor}; font-size: 16px;">${term}</b>
    </div>
    <div style="color: #E2E8F0; font-size: 14.5px; line-height: 1.6;">${def}</div>
    ${unit || dim ? `
    <div style="display: flex; gap: 16px; margin-top: 10px; padding-top: 8px; border-top: 1px dashed rgba(253, 200, 48, 0.3); font-size: 13.5px; color: #CBD5E1;">
      ${unit ? `<span><b>SI Unit:</b> <code style="color: #FFE082; background: rgba(0,0,0,0.3); padding: 1px 6px; border-radius: 4px;">${unit}</code></span>` : ""}
      ${dim ? `<span><b>Dimensional Formula:</b> <code style="color: #FFE082; background: rgba(0,0,0,0.3); padding: 1px 6px; border-radius: 4px;">${dim}</code></span>` : ""}
    </div>` : ""}
  </div>`;
}
...
function analogyBox(title, text) {
  ...
  <div style="background: rgba(14, 165, 233, 0.06); border-left: 4px solid #0EA5E9; border-radius: 6px; padding: 14px 18px; margin: 16px 0;">
    <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 6px;">
      ...
      <b style="color: #38BDF8; font-size: 15px;">Intuitive Analogy &bull; ${title}</b>
    </div>
...
function stepDerivationBox(title, steps) {
  ...
  const stepsHtml = steps.map((s, idx) => `
    <div style="display: flex; gap: 12px; margin-bottom: 12px; align-items: flex-start;">
      <span style="background: rgba(253, 200, 48, 0.2); color: ${themeColor}; font-weight: bold; width: 24px; height: 24px; border-radius: 50%; display: flex; align-items: center; justify-content: center; flex-shrink: 0; font-size: 12px; border: 1px solid ${themeColor};">${idx + 1}</span>
      <div style="flex: 1;">
  ...
  return `
  <div style="background: rgba(15, 23, 42, 0.6); border: 1px solid rgba(253, 200, 48, 0.3); border-radius: 10px; padding: 16px; margin: 18px 0;">
    <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 14px; border-bottom: 1px solid rgba(253, 200, 48, 0.2); padding-bottom: 8px;">
