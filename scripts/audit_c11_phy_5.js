...
const file = path.join(__dirname, '../client/data/content/c11-phy-5.ts');
const content = fs.readFileSync(file, 'utf-8');

console.log('=== AUDITING CLASS 11 PHYSICS CHAPTER 5 ===\n');
...
const mcqRegex = /"id": "c11-phy-5-mcq-(\d+)"/g;
...
const importCheck = ccContent.includes('c11Phy5Mcqs');
const mcqsAssigned = ccContent.includes('ch11Phy5.mcqs = c11Phy5Mcqs;');
const overviewAssigned = ccContent.includes('ch11Phy5.htmlOverview = c11Phy5HtmlOverview;');
const solutionsAssigned = ccContent.includes('c11Phy5HtmlSolutions');

if (importCheck && mcqsAssigned && overviewAssigned && solutionsAssigned) {
  console.log('✅ PASSED: chapterContent.ts is fully and correctly wired with Overview, Solutions, and MCQs for c11-phy-5.');
} else {
