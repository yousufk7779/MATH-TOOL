...
function embedImagesInFile(filePath, chapterDir) {
  ...
  content = content.replace(imgRegex, (match, prefix, src, suffix) => {
    ...
    if (fs.existsSync(path1) && fs.statSync(path1).isFile())
      finalImagePath = path1;
    else if (fs.existsSync(path2) && fs.statSync(path2).isFile())
      finalImagePath = path2;
    else if (fs.existsSync(path3) && fs.statSync(path3).isFile())
      finalImagePath = path3;
