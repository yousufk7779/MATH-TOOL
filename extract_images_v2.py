...
def extract_images(path):
    ...
    for i, p in enumerate(tree.iter('{http://schemas.openxmlformats.org/wordprocessingml/2006/main}p')):
        p_text = "".join(t.text for t in p.iter('{http://schemas.openxmlformats.org/wordprocessingml/2006/main}p') if t.text) # Wait this is wrong, should be t in p.iter(...t)
        # correction:
        p_text = ""
        ...
        if images:
            output.append({"p_index": i, "text": p_text, "images": images})

    document.close()
    return output

image_data = extract_images(r"d:\All NCERT SOLUTIONS\source_docs\science\physics\chapter3.docx")

with open('extracted_images.txt', 'w', encoding='utf-8') as f:
    for item in image_data:
        f.write(f"P{item['p_index']}: {item['text']}\n")
        for img in item['images']:
            f.write(f"IMAGE_B64: {img}\n")
        f.write("-" * 50 + "\n")

print(f"Extracted {len(image_data)} paragraphs with images.")
