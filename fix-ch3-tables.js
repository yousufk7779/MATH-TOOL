  ...
  return htmlArray.map((step) => {
    if (typeof step === "string" && step.includes("<table")) {
      ...
      tables.each((_, table) => {
        ...
        let newHtml =
          '<div style="background-color: rgba(255,255,255,0.05); border-radius: 8px; border-width: 1px; border-style: solid; border-color: rgba(255,255,255,0.1); margin-top: 10px; margin-bottom: 10px; overflow: hidden;">';
        trs.each((rIdx, tr) => {
      ...
      $("div.table-container").each((_, tc) => {
        $(tc).replaceWith($(tc).html());
      });
...
function traverse(obj) {
  if (obj.examples)
    obj.examples.forEach((ex) => {
      ex.solution = processTables(ex.solution);
    });
  if (obj.exercises)
    obj.exercises.forEach((ex) => {
      if (ex.questions)
        ex.questions.forEach((q) => {
          q.solution = processTables(q.solution);
        });
    });
}
