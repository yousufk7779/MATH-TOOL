...
const userDir = path.join(brainDir, '.user_uploaded');

const userImages = {
  skeleton_and_joints: 'media_1786725705903.png',
  neuron_and_reflex: 'media_1786725705925.png',
  endocrine_system: 'media_1786725705949.png'
};
...
async function prepareImages() {
  // 1. First 3 AI images
  for (const [key, baseName] of Object.entries(aiImages)) {
    const p = findImageFile(baseName);
    if (p) {
      const buf = fs.readFileSync(p);
      b64Data[key] = `data:image/jpeg;base64,${buf.toString('base64')}`;
      console.log(`Loaded 3D AI image for ${key}: ${buf.length} bytes`);
    }
  }

  // 2. Remaining 3 User-provided images enhanced with dark slate background
  for (const [key, filename] of Object.entries(userImages)) {
    const srcPath = path.join(userDir, filename);
    if (fs.existsSync(srcPath)) {
      // Process with sharp: add dark slate background fill to match 16:9 widescreen theme
      const buf = await sharp(srcPath)
        .resize(1280, 800, { fit: 'contain', background: { r: 13, g: 7, b: 34 } })
        .jpeg({ quality: 95 })
        .toBuffer();
      
      b64Data[key] = `data:image/jpeg;base64,${buf.toString('base64')}`;
      console.log(`Enhanced user-provided image for ${key}: ${buf.length} bytes`);
    } else {
      console.error(`Missing user uploaded file: ${srcPath}`);
    }
  }
...
function buildFile() {
  ...
  console.log("Generating c11-zoo-4.ts with exact user-provided images embedded...");
...
// High-Level Full Reference Book Content with Embedded HD Diagrams (No Captions Below Images)
...
`;
  ...
  console.log("Successfully assembled c11-zoo-4.ts with exact user-provided images!");
}

prepareImages().catch(err => console.error(err));
