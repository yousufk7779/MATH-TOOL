...
<div style="padding: 12px; color: #E0E0E0; text-align: justify; font-family: system-ui, -apple-system, sans-serif; line-height: 1.75; font-size: 16px;">
  ...
  <!-- DIAGRAM 1: CIRCUIT SYMBOLS -->
  ${imgCard('1', diags[13].title, diags[13].caption, diags[13].b64)}
  ...
  ${defBox('Electric Current (I)', `
    ...
    <b>Nature:</b> Electric current is a <b>scalar quantity</b> because it obeys ordinary scalar algebraic laws of addition rather than vector addition laws.
  `)}

  <!-- DIAGRAM 2 (IMAGE 1): ELECTRIC CURRENT -->
  ${imgCard('2', diags[1].title, diags[1].caption, diags[1].b64)}

  <h3 style="color: ${themeColor}; margin-top: 24px;">(i) Flow of Charges in Metallic Conductors &amp; Microscopic Electron Dynamics</h3>
  <p>In metallic conductors (such as copper, aluminium, and silver), valence electrons detach from positive metal ions to form an abundant free electron gas (number density <i>n &approx; 8.5 &times; 10<sup>28</sup> m<sup>-3</sup></i>). In thermal equilibrium without an external electric field:</p>
  ...
  ${stepDerivationBox('Relation Between Drift Velocity, Electric Current &amp; Current Density', `
    ...
    <b>Step 2 (Current in Terms of Drift Velocity, I = n e A v_d):</b> In a conductor of length <i>L</i> and cross-sectional area <i>A</i>, total number of free electrons is <i>N = n A L</i>, and total charge is <i>q = n A L e</i>.<br>
    ...
    <b>Step 3 (Current Density J):</b> Current per unit normal cross-sectional area:
    ${eqBox(`J = ${frac('I', 'A')} = n e v_d = n e \\left(${frac('e E \\tau', 'm')}\\right) = \\left(${frac('n e<sup>2</sup> \\tau', 'm')}\\right) E = <b>\\sigma E</b>`)}
  `)}

  <!-- DIAGRAM 3 (IMAGE 2): DRIFT VELOCITY & MOBILITY -->
  ${imgCard('3', diags[2].title, diags[2].caption, diags[2].b64)}
  ...
  ${defBox('Electron Mobility (&mu;)', `
    ...
    <b>SI Unit:</b> <b>m<sup>2</sup> V<sup>-1</sup> s<sup>-1</sup></b>.<br>
  ...
  <!-- SECTION 2 -->
  <h2 style="color: ${themeColor}; border-bottom: 2px solid ${themeColor}; padding-bottom: 6px; margin-top: 36px;">2. Ohm's Law, Electrical Resistance &amp; V–I Characteristics</h2>

  <h3 style="color: ${themeColor}; margin-top: 24px;">(i) Ohm's Law &amp; Its Deduction</h3>
  ...
  <!-- DIAGRAM 4 (IMAGE 3): OHM'S LAW -->
  ${imgCard('4', diags[3].title, diags[3].caption, diags[3].b64)}
  ...
  ${stepDerivationBox("Deduction of Ohm's Law from Drift Velocity", `
    ...
    At constant temperature, <i>m, n, e, &tau;</i> are constant for a given conductor, hence <b>R = constant</b>.
  `)}

  <h3 style="color: ${themeColor}; margin-top: 24px;">(ii) V–I Characteristics: Linear (Ohmic) &amp; Non-Linear (Non-Ohmic) Devices</h3>
  ...
  <!-- DIAGRAM 5 (IMAGE 4): V-I CHARACTERISTICS -->
  ${imgCard('5', diags[4].title, diags[4].caption, diags[4].b64)}

  <!-- SECTION 3 -->
  <h2 style="color: ${themeColor}; border-bottom: 2px solid ${themeColor}; padding-bottom: 6px; margin-top: 36px;">3. Electrical Resistivity, Conductivity, Temperature Dependence &amp; Combinations of Resistors</h2>

  <h3 style="color: ${themeColor}; margin-top: 24px;">(i) Electrical Resistivity (&rho;) and Conductivity (&sigma;)</h3>
  ...
  <p><b>Electrical Conductivity (&sigma;):</b> The reciprocal of resistivity: <b>&sigma; = 1 / &rho; = (n e<sup>2</sup> &tau;) / m</b> [SI Unit: Siemens per meter, S m<sup>-1</sup> or &Omega;<sup>-1</sup> m<sup>-1</sup>].</p>

  <!-- DIAGRAM 6 (IMAGE 5): RESISTIVITY AND CONDUCTIVITY -->
  ${imgCard('6', diags[5].title, diags[5].caption, diags[5].b64)}

  <!-- DIAGRAM 7 (IMAGE 15): RESISTIVITY OF MATERIALS TABLE -->
  ${imgCard('7', diags[15].title, diags[15].caption, diags[15].b64)}

  <h3 style="color: ${themeColor}; margin-top: 24px;">(ii) Temperature Dependence of Resistance &amp; Resistivity</h3>
  <p>As temperature rises, lattice ions vibrate more vigorously, increasing collision frequency and reducing relaxation time <i>&tau;</i>. Since <i>&rho; &prop; 1/&tau;</i>, resistance increases with temperature:</p>
  ...
  <!-- DIAGRAM 8 (IMAGE 6): TEMPERATURE DEPENDENCE -->
  ${imgCard('8', diags[6].title, diags[6].caption, diags[6].b64)}

  <h3 style="color: ${themeColor}; margin-top: 24px;">(iii) Combinations of Resistors (Series &amp; Parallel)</h3>
  <ul style="margin: 6px 0 6px 18px; line-height: 1.8;">
    <li><b>Series Combination:</b> Current <i>I</i> is identical across all resistors, voltages add up:
      ${eqBox(`<b>R_s = R_1 + R_2 + R_3 + ... + R_n</b>`)}
    </li>
    <li><b>Parallel Combination:</b> Potential difference <i>V</i> is identical across all branches, currents add up:
      ${eqBox(`<b>${frac('1', 'R_p')} = ${frac('1', 'R_1')} + ${frac('1', 'R_2')} + ${frac('1', 'R_3')} + ... + ${frac('1', 'R_n')}</b>`)}
    </li>
  </ul>

  <!-- DIAGRAM 9 (IMAGE 7): RESISTORS IN SERIES & PARALLEL -->
  ${imgCard('9', diags[7].title, diags[7].caption, diags[7].b64)}
  ...
  <!-- DIAGRAM 10 (IMAGE 14): ELECTRICAL ENERGY AND POWER -->
  ${imgCard('10', diags[14].title, diags[14].caption, diags[14].b64)}

  <!-- SECTION 5 -->
  <h2 style="color: ${themeColor}; border-bottom: 2px solid ${themeColor}; padding-bottom: 6px; margin-top: 36px;">5. Electromotive Force (EMF), Internal Resistance &amp; Terminal Potential Difference</h2>

  <h3 style="color: ${themeColor}; margin-top: 24px;">(i) Cell with Internal Resistance &amp; Circuit Equation</h3>
  <p>The finite resistance offered by the electrolyte and electrodes of a cell to the internal flow of current is its <b>internal resistance (r)</b>. For a cell of EMF <i>E</i> supplying current <i>I</i> to an external load resistor <i>R</i>:</p>

  ${eqBox(`<b>V = E - I r &implies; I = ${frac('E', 'R + r')} &implies; r = \\left(${frac('E', 'V')} - 1\\right) R</b>`)}

  <!-- DIAGRAM 11 (IMAGE 8): CELL WITH INTERNAL RESISTANCE -->
  ${imgCard('11', diags[8].title, diags[8].caption, diags[8].b64)}

  <h3 style="color: ${themeColor}; margin-top: 24px;">(ii) EMF vs Terminal Potential Difference</h3>
  <p><b>Electromotive Force (EMF, E):</b> The maximum potential difference across the terminals of a cell in an <b>open circuit</b> (no current drawn, <i>I = 0 &implies; V = E</i>).<br>
  <b>Terminal Voltage (V):</b> The potential difference between cell terminals during <b>closed circuit discharge</b> (current drawn, <i>V = E - I r &lt; E</i>).</p>

  <!-- DIAGRAM 12 (IMAGE 9): EMF AND TERMINAL VOLTAGE -->
  ${imgCard('12', diags[9].title, diags[9].caption, diags[9].b64)}

  <!-- SECTION 6 -->
  <h2 style="color: ${themeColor}; border-bottom: 2px solid ${themeColor}; padding-bottom: 6px; margin-top: 36px;">6. Kirchhoff's Rules, Wheatstone Bridge Principle &amp; Meter Bridge</h2>

  <h3 style="color: ${themeColor}; margin-top: 24px;">(i) Kirchhoff's Two Laws</h3>
  ${defBox("Kirchhoff's Laws", `
    <b>1. Kirchhoff's First Rule (Junction Rule / KCL):</b> The algebraic sum of all currents meeting at any electrical junction is zero:
    ...
    <b>2. Kirchhoff's Second Rule (Loop Rule / KVL):</b> The algebraic sum of potential changes (&Delta;V) around any closed loop is zero:
    ${eqBox(`<b>\\Sigma \\Delta V = \\Sigma E + \\Sigma (I R) = 0</b>`)}
    <i>(Direct manifestation of the Law of Conservation of Energy).</i>
  `)}

  <!-- DIAGRAM 13 (IMAGE 10): KIRCHHOFF'S RULES -->
  ${imgCard('13', diags[10].title, diags[10].caption, diags[10].b64)}

  <h3 style="color: ${themeColor}; margin-top: 24px;">(ii) Wheatstone Bridge Principle</h3>
  <p>The Wheatstone bridge consists of four resistors <i>P, Q, R, S</i> arranged in a bridge network with a central galvanometer <i>G</i>. At null balance (no deflection, <i>I<sub>g</sub> = 0</i>):</p>

  ${eqBox(`<b>${frac('P', 'Q')} = ${frac('R', 'S')}</b>`)}

  <!-- DIAGRAM 14 (IMAGE 11): WHEATSTONE BRIDGE -->
  ${imgCard('14', diags[11].title, diags[11].caption, diags[11].b64)}

  <h3 style="color: ${themeColor}; margin-top: 24px;">(iii) Meter Bridge (Slide Wire Practical Bridge)</h3>
  <p>The <b>meter bridge</b> is the practical laboratory apparatus based on the balanced Wheatstone bridge principle, using a 1-meter uniform resistance wire and a sliding jockey. When balanced at balancing length <i>l</i> cm from the left zero end:</p>

  ${eqBox(`<b>${frac('R', 'X')} = ${frac('l', '100 - l')} &implies; X = ${frac('100 - l', 'l')} R</b>`)}

  <!-- DIAGRAM 15 (IMAGE 12): METER BRIDGE -->
  ${imgCard('15', diags[12].title, diags[12].caption, diags[12].b64)}
...
console.log("Option 1 applied successfully! All 15 diagrams are now in 100% sequential chronological order (1 to 15)!");
