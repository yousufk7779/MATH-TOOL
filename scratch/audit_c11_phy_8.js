...
const filePath = path.join(__dirname, '..', 'client', 'data', 'content', 'c11-phy-8.ts');
const content = fs.readFileSync(filePath, 'utf8');

const matchOverview = content.match(/export const c11Phy8HtmlOverview = ([\s\S]*?);\n\nexport const/);
const matchSol = content.match(/export const c11Phy8HtmlSolutions = ([\s\S]*?);?\s*$/);

const overview = JSON.parse(matchOverview[1]);
const sol = JSON.parse(matchSol[1]);

console.log("=== CHAPTER 8 (THERMODYNAMICS) AUDIT ===");
