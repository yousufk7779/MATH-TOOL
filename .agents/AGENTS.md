...
4. **Theme Color & Question Hierarchy**: Main question headers (`Question 1`) in `themeColor`, full question statement in pure `#FFFFFF`, sub-parts `(i)` in `themeColor` followed by `#FFFFFF` problem text, card left border in `themeColor`, and final answer in a crisp `#4CAF50` green box.
