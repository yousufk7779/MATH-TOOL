...
with open('full_image_details.txt', 'w', encoding='utf-8') as f:
    for item in image_data:
        f.write(f"--- PARAGRAPH P{item['index']} ---\n")
        f.write(f"TEXT: {item['text'][:100]}...\n")
        f.write(f"NUM_IMAGES: {len(item['images'])}\n")
        for j, img in enumerate(item['images']):
            f.write(f"IMAGE_{j}_B64_START: {img[:100]}...\n")
            f.write(f"IMAGE_{j}_B64_END: {img[-100:]}\n")
