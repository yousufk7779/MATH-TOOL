...
def find_id(p_id, text):
    if not text:
        return "N/A"
    # Find surrounding text to identify question
    # Search for text in ts
    # Clean text for search (remove Ω etc which might be &Omega; in ts)
    search_text = text.replace("Ω", "&Omega;").replace("'", "’")
    # Take first 50 chars
    search_text = search_text[:50]
    
    # Very basic search
    match = re.search(r'"id": "([^"]+)"[^}]+' + re.escape(search_text), ts_content, re.S)
    if match:
        return match.group(1)
    
    # Reverse search - find the LAST id before this text
    idx = ts_content.find(search_text)
    if idx != -1:
